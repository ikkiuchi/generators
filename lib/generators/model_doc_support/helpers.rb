  ...
  module Helpers
    ...
    def inherited(base)
      ...
      base.class_eval do
        ...
        self.model_name = name.sub('ModelDocs::', '')
        self.model_rb_stack = [ '' ]
    ...
    def process_and_returns_options(name, type, req, options)
      builder_rmv << name if options.delete(:not_show)
    ...
    def _fields
      Hash[fields.map { |name, info| [name, info.drop(1).reduce({ }, :merge)] }]
    end

    def process_validates
      %i[ null presence absence uniqueness ].each do |key|
        names = _fields.map { |name, info| name if !info[key].nil? && name.is_a?(Symbol) }.compact # str name is belongs_to
        next if names.blank?
        (to_single_validates[names.first] ||= '') << ", #{key}: true" and next if names.size == 1 && key != :null
    ...
    def run
      Dir['./app/_docs/others/model_docs/*.rb'].each { |file| require file }
      descendants.each do |mdoc|
        ...
        doc_path = "app/_docs/api/#{mdoc.doc_version}/#{model_file_name.pluralize}#{mdoc.version}_doc.rb"

        if Generators::ModelDocSupport::Config.overwrite || !File::exist?(model_path)
        # if true
          mdoc.fields_to_migration
          mdoc.fields_to_fbot
          puts ['    ', mdoc.model_name.rjust(10), ' # Model Doc'].join.blue.sub('34', '36')
          write :Model, mdoc.model_rb.sub("\n\n\nend\n", "\nend\n"), to: model_path
          write :Migration, mdoc.migration_rb.sub("    XXX\n", ''), to: mg_path
    ...
    def fields_to_migration
      ...
      key_order = %i[ foreign_key polymorphic array null default index unique ] # TODO
    ...
    def migration_rb
      <<~MG
        class Create#{model_name.pluralize} < ActiveRecord::Migration::Current
          def change
            create_table :#{model_name.underscore.pluralize}, force: :cascade do |t|
    ...
    def doc_rb
      ...
      <<~DOC
        ...
        class Api::#{doc_version.upcase}::#{model_name.pluralize}Doc# < ApiDoc
          ...
          api :create, 'POST create a #{name}' do
            form! data: {
            }
          end

          api :update, 'PATCH update the specified #{name}', use: id do
            form! data: {
            }
          end

          api :destroy, 'DELETE the specified #{name}', use: id
...
(6) presence - not_blank & absence - should_blank
